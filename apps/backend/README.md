...
- **`GET /api/swapi/people`** — proxies [SWAPI](https://swapi.dev/api/people)'s people list.
  ...
  - The `starships` and `vehicles` arrays (SWAPI urls, e.g. `["https://swapi.dev/api/starships/12/", ...]`) are each replaced with just the numeric ids (`["12", ...]`), for use with `GET /api/swapi/starships/:id` and `GET /api/swapi/vehicles/:id` respectively.
  - `homeworld` (a single SWAPI url) is likewise reduced to just its numeric id, for use with `GET /api/swapi/planets/:id`. Other reference arrays (`films`, `species`) are left as-is.
...
- **`GET /api/swapi/planets`** / **`GET /api/swapi/planets/:id`** — same shape again (`planet_id` instead of `url`), proxying [SWAPI's planets](https://swapi.dev/api/planets) and enriching against the Databank `locations` category (`data/locations.json`), matched by `name` (no `model` fallback, since planets don't have one).
  - The `residents` array (SWAPI urls) is replaced with just the numeric character ids, for use with `GET /api/swapi/people/:id`. Other reference arrays (`films`) are left as-is.
