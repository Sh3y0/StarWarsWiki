...
| **Mobile**  | `[apps/mobile](apps/mobile/README.md)`   | Expo + React Native + TypeScript | List + detail screens for Characters, Films, Starships, Vehicles, Planets |
...
Expo + React Native + TypeScript app consuming the backend above. Expo Router with a bottom-tab navigator for 6 categories (Characters, Films, Starships, Vehicles, Species, Planets), styled-components for styling, TanStack Query for data fetching, Zod for validating API responses, expo-image for remote images.

Each of the 5 tabs has a searchable list and a detail screen, mapped from the Stitch **Star Wars Galactic Archives** designs onto the backend SWAPI proxy (with Databank images/copy where available, and local posters for films).
...
Mobile: unit tests for UI, catalog mappers, and the `useCatalogList` hook (mocking the API layer), using Jest + `@testing-library/react-native`.